...
<script type="text/javascript">
	...
	function searchAll(){
		//$('#mode').val('-');
		//$('#keyword').val('');
		location.href='<%=contextPath%>/boList';
	}
</script>
</head>
<body>
				...
				<tr>
					<td colspan="10" align="center">
						<form class="form-inline" role="form" name="myform" action="<%=MyCtrlCommand%>boList" method="post">
							...
							<div class="form-group">
								<input type="text" class="form-control btn-ms" name="keyword"
				...
				<c:forEach var="bean" items="${requestScope.lists}">
					<tr>
						...
						<td>
							...
							<a href="<%=MyCtrlCommand%>boDetailView&no=${bean.no}&${requestScope.parameter}">
								${bean.subject}
							</a>
						</td>
						<td>${bean.password}</td>
						...
						<td>
							<c:if test="${sessionScope.loginfo.id == bean.writer}">
								<a href="<%=MyCtrlCommand%>boUpdate?no=${bean.no}&${requestScope.parameter}">
									수정
								</a>
							</c:if>
							<c:if test="${sessionScope.loginfo.id != bean.writer}">
						...
						<td>
							<c:if test="${sessionScope.loginfo.id == bean.writer}">
								<a href="<%=MyCtrlCommand%>boDelete?no=${bean.no}&${requestScope.parameter}">
									삭제
								</a>
							</c:if>
							<c:if test="${sessionScope.loginfo.id != bean.writer}">
						...
						<td>
							<a href="<%=MyCtrlCommand%>boReply?no=${bean.no}&${requestScope.parameter}&groupno=${bean.groupno}&orderno=${bean.orderno}&depth=${bean.depth}">
								답글 
							</a>
						</td>
					</tr>
